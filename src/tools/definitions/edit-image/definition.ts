...
KREA IDENTITY EDIT: Use model="krea-identity-edit" for Krea 2 Identity Edit LoRA v1.2, identity-preserving Krea edits, and the krea2_identity_edit_v1_2 model id. Use model="dark-beast-krea2-identity-edit" for Dark Beast Krea 2 Identity Edit or the dark_beast_krea2_identity_edit_v1_2 model id. These models require one reference image, accept up to two context images, work best at 512-2048 px, default to 10 steps with guidance 1, and do not use negative prompts. Put the primary scene/base image first and the person/detail reference second for scene-plus-person edits; reference them with context_image_0 and context_image_1 when model_ref tokens are needed.
...
COMPOSITE GPT IMAGE 2 STORYBOARD SHEETS: When numberOfVariations=1 and the user asks for one composite video storyboard/keyframe sheet using uploaded or generated references, the prompt must be a compiled storyboard prompt, not a concept summary. Include a SCENES: section with exactly the requested number of concrete entries named SCENE_01, SCENE_02, etc. Every scene entry must include Visual/Action, Camera/Motion, Dialogue/VO (or [no dialogue]), Audio/SFX, and any visible text or reference usage for that scene. Do not provide only the source brief or generic layout instructions; malformed compiled storyboard prompts are blocked by quality audit.`,
        ...
        model: {
          ...
          description:
            'The app auto-selects the edit model from the Default Media Quality setting (Fast→Qwen Lightning, HQ→full Qwen, Pro→Flux.2) as a FALLBACK for ordinary edits. That default is a floor, not a cap: set this parameter to override it whenever the task needs more fidelity than the fallback gives, and do not let a Fast default downgrade a fidelity-critical edit. Set "gpt-image-2" when the user names a ChatGPT, OpenAI, GPT, GPT-2, GPT Image, or gpt-image-2 model, OR for any detail-critical render or edit — dense labels, crisp or legible typography, fine text, multi-panel composition, timing notes, foley notes, professional storyboard-sheet/contact-sheet/panel layout, or a character/mascot/model sheet with turnarounds, expressions, accessories, palette swatches, and brand notes — INCLUDING editing an uploaded image that is itself a storyboard, contact sheet, multi-panel layout, or text-dense graphic. Choose "gpt-image-2" for these even when the Default Media Quality is Fast. If "gpt-image-2" is unavailable (for example insufficient credits), fall back to "qwen" (full Qwen Image Edit), never "qwen-lightning", for such fine-detail or legible-text edits. Set "krea-identity-edit" when the user asks for Krea 2 Identity Edit LoRA v1.2, Krea identity edit, identity-preserving Krea edits, face/likeness preservation with one or two references, or the krea2_identity_edit_v1_2 model id. Set "dark-beast-krea2-identity-edit" when the user asks for Dark Beast Krea 2 Identity Edit, its community/uncensored identity edit variant, or the dark_beast_krea2_identity_edit_v1_2 model id. Krea identity edit models require at least one reference image, accept up to two context images, work best at 512-2048px, use about 8-12 steps with default steps=10 and guidance=1, and do not use negative prompts. For ordinary single-subject edits with no fine text or panels, omit this parameter and let the quality default apply. Z-image, Z-image Turbo, and Krea 2 Turbo are not edit_image models; route those explicit uploaded-image enhancement requests to generate_image with sourceImageIndex and starting_image_strength. If the user names another edit/image model, honor that requested model instead. GPT Image 2 always processes input images at high fidelity; do not set input_fidelity.',
        },
        sourceImageIndex: {
          type: 'number',
          description:
