/**
 * Tool definition for overlay_video.
 *
 * Burns text and/or image (logo, watermark) overlays onto a previously
 * rendered or uploaded video. Pure ffmpeg — no model inference. Each overlay
 * is positioned with a keyword (corner / edge / center) plus an optional
 * pixel offset, and may be limited to a time range so e.g. a logo only
...
export const definition: ToolDefinition = {
  ...
  function: {
    name: 'overlay_video',
    description:
      'Burn text and/or logo/watermark image overlays onto a previously rendered or uploaded video. ' +
    ...
    parameters: {
      ...
      properties: {
        sourceVideoIndex: {
          type: 'number',
          description:
            'Which video to overlay onto. Omit to use the most recent generated video, or the first uploaded video when no generated video exists. ' +
        ...
        overlays: {
          ...
          minItems: 1,
          description:
            'Ordered list of overlays to burn in. Each overlay is rendered on top of all previous overlays. ' +
          ...
          items: {
            ...
            properties: {
              ...
              position: {
                ...
                description:
                  'Anchor position on the frame. Pixel offsets nudge inward; the renderer pads ' +
              ...
              startSeconds: {
                type: 'number',
                description:
                  'Show the overlay from this time. Default 0 (show from the start). ' +
                  'Negative values are relative to the end of the base video; startSeconds=-2 means start 2 seconds before the end.',
              },
              endSeconds: {
                type: 'number',
                description:
                  'Hide the overlay at this time. Default = full video duration. ' +
              ...
              fontSizePct: {
                ...
                maximum: 30,
                description:
                  'Font size as a percentage of the video height. Default: 6 (≈ 43px on a 720p frame). ' +
                  'Only valid when kind="text".',
              },
              color: {
                type: 'string',
                description:
                  'Text fill color (CSS hex like "#FFFFFF" or named ffmpeg color). Default "#FFFFFF". ' +
                  'Only valid when kind="text".',
              },
              outlineColor: {
                type: 'string',
                description:
                  'Text outline color. Default "#000000" with a thin stroke for legibility. ' +
                  'Only valid when kind="text".',
              },
              backgroundColor: {
                type: ['string', 'null'],
                description:
                  'Optional rgba background pill behind the text (e.g. "rgba(0,0,0,0.5)"). null = no box. ' +
                  'Only valid when kind="text".',
              },
              fontWeight: {
                type: 'string',
              ...
              sourceImageIndex: {
                type: 'number',
                description:
                  'Which image to overlay. Required when kind="image". ' +
              ...
              widthPct: {
                ...
                maximum: 100,
                description:
                  'Logo width as a percentage of the video width. Default: 15. ' +
              ...
              fit: {
                ...
                enum: ['contain', 'cover'],
                description:
                  'Image sizing mode. Default "contain" scales by widthPct and preserves the full overlay image. ' +
