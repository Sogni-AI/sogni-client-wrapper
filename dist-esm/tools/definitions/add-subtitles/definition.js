export const SUBTITLE_VERTICAL_POSITIONS = ['bottom', 'top', 'center'];
export const definition = {
    type: 'function',
    function: {
        name: 'add_subtitles',
        description: 'Burn subtitles into a video from caller-supplied cues or an SRT/VTT string. ' +
        ...
        parameters: {
            ...
            properties: {
                sourceVideoIndex: {
                    type: 'number',
                    description: 'Which video to subtitle. Default: -1 (most recent generated or uploaded video). ' +
                        'Non-negative values are 0-based indices into prior generated video results. ' +
                        'Negative values reference uploaded videos.',
                },
                cues: {
                    type: 'array',
                    description: 'Ordered subtitle cues. Each cue has startSeconds, endSeconds, and the line of text to display. ' +
                ...
                srt: {
                    type: 'string',
                    description: 'Full SRT (or VTT) document as a string, used in place of `cues`. Useful when the user pastes ' +
                        'a subtitle file directly. Provide either `cues` or `srt`, not both.',
                },
                auto_transcribe: {
                    type: 'boolean',
                    description: 'Reserved for future speech-to-text support. Currently returns USER_INPUT_INCOMPLETE so the LLM ' +
