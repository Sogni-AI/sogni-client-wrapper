import { emptyTurnPolicy } from './turnPolicy.js';
export function classifyTurn(input) {
    ...
    const signalSourcesByKind = new Map();
    for (const signal of policy.signals) {
        const sources = signalSourcesByKind.get(signal.kind) ?? new Set();
        sources.add(signal.source);
        signalSourcesByKind.set(signal.kind, sources);
    }
    const presentSignalKinds = new Set(signalSourcesByKind.keys());
    const signalMatchesSourceConstraint = (kind, allowed) => {
        if (!presentSignalKinds.has(kind))
            return false;
        if (!allowed)
            return true;
    ...
    const rationales = [];
    const firedTelemetryPayloads = [];
    for (const gating of input.registry.listGatingPolicies()) {
        const allOfMatch = gating.trigger.allOf.every((kind) => signalMatchesSourceConstraint(kind, gating.trigger.sources?.[kind]));
        const noneOfMatch = !gating.trigger.noneOf || gating.trigger.noneOf.every((kind) => !presentSignalKinds.has(kind));
        ...
        if (gating.effect.require) {
            for (const tool of gating.effect.require)
...
export function compileToolsForTurn(input) {
    ...
    const contextLines = [];
    if (input.turnPolicy.rationale)
        contextLines.push(input.turnPolicy.rationale);
...
function autoRepairFieldsForRecipe(recipe) {
...
function autoRepairValue(field, recipe, ctx) {
...
function sessionStateSignals(input) {
    const signals = [];
...
function sessionStateHints(input) {
    const state = input.sessionState;
    const facts = [];
    if (state.hasPriorGenerationContext)
        facts.push('prior generated context exists');
    if (state.hasGeneratedImage)
        facts.push('generated images are available');
    if (state.hasGeneratedVideo)
        facts.push('generated videos are available');
    if (state.hasGeneratedAudio)
        facts.push('generated audio is available');
    if (state.hasUploadedImage)
        facts.push('uploaded images are available');
    if (state.hasUploadedVideo)
        facts.push('uploaded videos are available');
    if (state.hasUploadedAudio)
        facts.push('uploaded audio is available');
    if (state.hasActivePersona)
        facts.push('an active persona is in scope');
    if (state.noPersonaImageInSession)
        facts.push('no persona image stage is recorded yet');
    if (state.awaitingImageSelection)
        facts.push('waiting for image selection before video');
    if (state.pendingStitchAfterBatch)
        facts.push('a generated batch is awaiting stitch');
    if (state.completedWorkflow)
        facts.push('a media workflow completed this turn');
    if (state.lastTool)
        facts.push(`last tool: ${state.lastTool}`);
    if (state.lastToolError)
        facts.push(`last tool error: ${state.lastToolError}`);
    if (state.repairCount > 0)
        facts.push(`repair attempts this turn: ${state.repairCount}`);
    if (facts.length === 0)
        return [];
    return [{
            kind: 'session_state',
...
function isPromptUnsafeCodePoint(code) {
    if (code <= 0x1f)
        return true;
    if (code === 0x7f)
        return true;
...
function sanitizeAssetIdForHint(id) {
    let stripped = '';
...
function assetManifestHints(input) {
    ...
    return [{
            kind: 'asset_manifest',
            body: `Assets in scope: ${items.length} total${countSummary ? ` (${countSummary})` : ''}${ids ? `. Use manifest ids when referencing assets: ${ids}.` : '.'}`,
        }];
}
function isRecord(value) {
    return value !== null && typeof value === 'object' && !Array.isArray(value);
}
function applyParameterDocs(parameters, docs) {
...
function applyAutoRepairFields(args, recipe, ctx) {
...
function repairAttemptCount(input, recipe) {
...
function hasRepairBudget(input, recipe) {
    if (recipe.mode === 'stopAndAsk')
        return true;
...
function computeDispatch(input) {
    if (input.turnPolicy.forbiddenTools.includes(input.call.name)) {
        return {
            result: {
                kind: 'reject',
                rationale: input.turnPolicy.rationale,
                gatingPolicyId: input.turnPolicy.forbiddenToolPolicies[input.call.name] ??
        ...
        if (recipe) {
            ...
            if (!hasRepairBudget(input, recipe)) {
                return {
                    result: {
                        kind: 'ask_user',
                        userQuestion: `I could not safely recover after ${recipe.maxRetries} repair attempt${recipe.maxRetries === 1 ? '' : 's'}. ${note}`,
            ...
            switch (recipe.mode) {
                case 'autoRepair':
                    return {
                        result: {
                            ...
                            args: applyAutoRepairFields(input.call.arguments, recipe, input.noteContext ?? {}),
                            repairNote: note,
                            recipeId: recipe.recipeId,
                        },
                        firedRecipe: recipe,
                ...
                default: {
                    const _exhaustive = recipe.mode;
                    throw new Error(`Unhandled RepairRecipeMode: ${_exhaustive}`);
                }
            }
        }
    }
