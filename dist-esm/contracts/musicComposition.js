import { LANGUAGE_LABELS, MUSIC_BPM, MUSIC_DURATION, MUSIC_KEY_SCALES, MUSIC_TIME_SIGNATURES, } from '../media/musicSettings.js';
...
export const LYRICS_COMPOSITION_TOOL = {
    type: 'function',
    function: {
        name: 'compose_lyrics',
        ...
        parameters: {
            ...
            properties: {
                lyrics: {
                    type: 'string',
                    description: 'Song lyrics with enriched section headers (e.g. [Verse 1 - soft vocal]). Use \\n for newlines.',
                },
...
export const INSTRUMENTAL_COMPOSITION_TOOL = {
    type: 'function',
    function: {
        ...
        parameters: {
            ...
            properties: {
                structure: {
                    type: 'string',
                    description: 'Instrumental structure as ONLY bracketed section tags, one per line, with NO text between them (e.g. [Intro - ambient pads]\\n[Main Theme - driving guitar]\\n[Bridge - stripped back]). Use \\n for newlines.',
                },
                bpm: {
                    type: 'number',
...
export function buildLyricsMessages(prompt, language, musicPrompt, randomTheme) {
    const messages = [{ role: 'system', content: LYRICS_SYSTEM_PROMPT }];
    let userMessage;
    if (prompt.trim()) {
        userMessage = prompt;
    }
...
export function buildInstrumentalMessages(prompt, musicPrompt, randomTheme) {
    const messages = [{ role: 'system', content: INSTRUMENTAL_SYSTEM_PROMPT }];
    let userMessage;
    if (prompt.trim()) {
        userMessage = prompt;
    }
...
function parseTimeSignature(value) {
    ...
    const map = { '2/4': '2', '3/4': '3', '4/4': '4', '6/8': '6' };
    if (map[str])
        return map[str];
    const signatures = Object.keys(MUSIC_TIME_SIGNATURES);
    if (signatures.includes(str))
        return str;
    return null;
}
function parseTempo(value) {
    const num = Number(value);
    if (!isNaN(num) && num >= MUSIC_BPM.min && num <= MUSIC_BPM.max)
        return Math.round(num);
    return null;
}
function parseDuration(value) {
    const num = Number(value);
    if (!isNaN(num) && num >= MUSIC_DURATION.min && num <= MUSIC_DURATION.max)
        return Math.round(num);
    return null;
}
function parseKeyScale(value) {
    if (!value)
        return null;
    const str = String(value);
...
export function parseToolCallResult(args) {
...
//# sourceMappingURL=musicComposition.js.map