...
BATCH VARIATIONS: When numberOfVariations > 1, use Dynamic Prompt syntax to vary the visual interpretation while keeping audio sync intent consistent. This is one Sogni project with multiple jobs, so prefer it when all outputs share the same audio source/window, image source, model, duration, dimensions, and parameters and only prompt text varies. Example: "{abstract neon visualization|nature scene with swaying trees|urban street with rain} synced to the beat".`,
        ...
        videoModel: {
          ...
          description:
            'Video model. "ltx23-ia2v" (default when image available): LTX 2.3 image+audio to video, audio-reactive with a reference image; Fast/HQ use the distilled 8-step worker and Default Media Quality Pro uses the non-distilled dev worker. "ltx23-a2v" (default when no image): LTX 2.3 audio-only to video, no image needed, creates video purely from text prompt + audio with the same quality-tier routing. "wan-s2v": WAN 2.2 sound-to-video, best for lip-sync with a face image, fast 4-step. "seedance2": full Seedance 2.0 audio-reference video, 4-15s; this tool supplies the audio plus a required reference image because Seedance text+audio without image/video is unsupported. "seedance2-fast": Seedance 2.0 Fast for faster/lower-cost iteration. Seedance quality is selected only by this model value: pick "seedance2-fast" when the user says Seedance Fast / seedance-fast / faster draft, and pick "seedance2" for full/non-fast Seedance or 1080p. Do not infer the Seedance model from Default Media Quality Fast/HQ/Pro. ' +
            SEEDANCE_TOOL_AUDIO_REFERENCE_GUIDANCE +
        ...
        generateAudio: {
          type: "boolean",
          description:
            "Seedance only. Whether Seedance should include a generated/native audio track in the final video. Omit by default so the reference audio drives the result; set false only for explicit silent output.",
        },
        numberOfVariations: {
          type: "number",
          description:
