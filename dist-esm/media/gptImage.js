...
export const GPT_IMAGE_DIMENSION_BOUNDS = {
    minDimension: 256,
    maxDimension: 3840,
    maxAspectRatio: 3,
...
export const GPT_IMAGE_QUALITY_BY_TIER = {
    fast: 'low',
    hq: 'medium',
    pro: 'high',
};
...
export const GPT_IMAGE_MODEL_ALIASES = new Set(GPT_IMAGE_MODEL_ALIAS_VALUES);
...
export function normalizeGptImageModelAlias(value) {
    if (typeof value !== 'string')
        return undefined;
    const normalized = value.trim().toLowerCase().replace(/[_\s]+/g, '-');
    if (GPT_IMAGE_MODEL_ALIASES.has(normalized))
        return GPT_IMAGE_MODEL_KEY;
    return value;
}
export function textRequestsGptImage2ImageModel(text) {
    if (!text)
        return false;
...
export function textExplicitlyAvoidsGptImageModel(text) {
...
export function textExplicitlyRequestsNonGptImageModel(text) {
    if (!text || textRequestsGptImage2ImageModel(text))
        return false;
    const versionedModel = new RegExp(String.raw `\b(?:use|using|with|via|through|choose|select|switch\s+to|model|renderer)\b[\s\S]{0,60}\b${VERSIONED_NON_GPT_IMAGE_MODEL_NAME_PATTERN}\b|\b${VERSIONED_NON_GPT_IMAGE_MODEL_NAME_PATTERN}\b[\s\S]{0,40}\b(?:model|renderer)\b`, 'i');
    if (versionedModel.test(text))
        return true;
    const familyModel = new RegExp(String.raw `\b(?:use|using|with|via|through|choose|select|switch\s+to)\s+(?:the\s+)?${FAMILY_NON_GPT_IMAGE_MODEL_NAME_PATTERN}\b(?=\s*(?:to|for|as|(?:image\s+)?(?:model|renderer)\b|$|[.,;:]))|\b${FAMILY_NON_GPT_IMAGE_MODEL_NAME_PATTERN}\s+(?:image\s+)?(?:model|renderer)\b`, 'i');
    if (familyModel.test(text))
        return true;
...
function textMentionsImageRenderIntent(text) {
...
function textRequestsVideoFromStoryboardReference(text) {
...
export function textSuggestsGptImage2DefaultImageModel(text) {
    if (!text || textRequestsVideoFromStoryboardReference(text))
        return false;
    const hasImageIntent = textMentionsImageRenderIntent(text);
    if (!hasImageIntent)
        return false;
    const asksForStoryboardImage = /\b(?:story\s*board|storyboard|shot\s*sheet|sequence\s*sheet|thumbnail\s*sequence|panel\s*(?:layout|grid|sheet)|multi[-\s]?panel|contact\s*sheet|mood\s*board|moodboard)\b/i.test(text)
    ...
    const asksForVideoStoryboardImage = /\b(?:generate|create|make|render|produce|design|build|develop|draw)\b[\s\S]{0,180}\b(?:video\s+)?(?:story\s*board|storyboard)(?:\s+(?:sequence|sheet|layout|panel|panels|board))?\b/i.test(text)
    ...
    const asksForComplexRender = /\b(?:very|highly|extremely|especially|super)\s+(?:complex|detailed|intricate|elaborate)\b[\s\S]{0,80}\b(?:images?|renders?|illustrations?|artwork|posters?|graphics?|story\s*boards?|storyboards?|contact\s*sheet|mood\s*board|moodboard|shot\s*sheet|sequence\s*sheet|panel\s*(?:layout|grid|sheet))\b/i.test(text)
    ...
    const asksForTextOrLayoutPrecision = /\b(?:crisp|readable|precise|clean)\s+(?:typography|text|labels?|caption|captions|callouts?)\b/i.test(text)
...
export function getGptImage2ModelOverride(toolName, currentModel, latestUserText) {
    if (!GPT_IMAGE_TOOL_NAMES.has(toolName))
        return null;
    if (textExplicitlyAvoidsGptImageModel(latestUserText))
        return null;
    ...
    const shouldUseComplexRenderDefault = !explicitlyRequestedGptImage
...
export function normalizeGptImageQuality(value) {
    if (typeof value !== 'string')
        return undefined;
    const normalized = value.trim().toLowerCase();
    if (normalized === 'auto')
        return 'auto';
    if (['low', 'fast', 'draft', 'quick'].includes(normalized))
        return 'low';
    if (['medium', 'standard', 'balanced', 'hq'].includes(normalized))
        return 'medium';
    if (['high', 'pro', 'best', 'final'].includes(normalized))
        return 'high';
    return undefined;
}
...
const HIGH_DETAIL_STORYBOARD_PATTERN = /\b(?:story\s*board|storyboard|panel|panels|scene\s+cell|caption|captions|timing labels?|shot labels?|foley notes?|text rendering)\b/i;
const HIGH_DETAIL_SMALL_TYPE_PATTERN = /\b(?:small type|readable labels?|concise labels?|visible text|scene number|timing|captions?|brand copy|cta|logo|wordmark)\b/i;
export function promptNeedsHighDetailGptImage(prompt) {
    const text = prompt ?? '';
    return HIGH_DETAIL_STORYBOARD_PATTERN.test(text) && HIGH_DETAIL_SMALL_TYPE_PATTERN.test(text);
}
//# sourceMappingURL=gptImage.js.map