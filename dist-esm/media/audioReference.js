...
export function detectReferenceAudioFormat(data, mimeType) {
    ...
    if (data.length >= 4 && asciiAt(data, 0, 4) === 'fLaC')
        return 'flac';
    if (data.length >= 4 && asciiAt(data, 0, 4) === 'OggS')
        return 'ogg';
    return 'unknown';
}
function shouldNormalizeReferenceAudioToM4a(sourceFormat, options) {
    if (sourceFormat === 'mp3')
        return true;
...
function shouldNormalizeReferenceAudioToMp3(sourceFormat, options) {
...
export function shouldTrimSeedanceReferenceAudio(input) {
    ...
    if (startOffset > 0)
        return true;
    const maxDuration = finitePositive(input.maxDurationSeconds, SEEDANCE_R2V_REFERENCE_AUDIO_MAX_DURATION_SECONDS);
...
export async function prepareReferenceAudioForVideo(data, mimeType, filename = 'reference-audio', options = {}) {
...
export async function prepareSeedanceReferenceAudioForVideo(data, mimeType, filename = 'reference-audio', options = {}) {
    const inputMimeType = normalizeReferenceAudioMimeType(mimeType);
    const sourceFormat = detectReferenceAudioFormat(data, inputMimeType);
    const sourceDuration = typeof options.sourceDurationSeconds === 'number'
        ...
        : undefined;
    const requestedDuration = finitePositive(options.maxDurationSeconds, SEEDANCE_R2V_REFERENCE_AUDIO_MAX_DURATION_SECONDS);
    ...
    const prepared = await prepareReferenceAudioForVideo(data, inputMimeType, filename, options);
    return {
        ...prepared,
        requestedDuration,
        startOffset,
...
//# sourceMappingURL=audioReference.js.map