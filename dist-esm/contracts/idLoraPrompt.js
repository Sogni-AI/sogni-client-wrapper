...
export const ID_LORA_COMPOSITION_TOOL = {
    type: 'function',
    function: {
        ...
        parameters: {
            ...
            properties: {
                visual: {
                    type: 'string',
                    description: 'Scene, subject appearance, lighting, camera, and visible action. No dialogue, no ambient sound.',
                },
                speech: {
                    type: 'string',
                    description: 'Literal words spoken by the subject, verbatim, no quotes. Empty string if the source has no speech.',
                },
                sounds: {
                    type: 'string',
                    description: "Speaker vocal qualities (volume, tone, mic proximity) followed by ambient/background audio.",
                },
            },
...
export function buildIdLoRaConversionMessages(ltxPrompt) {
    return [
...
export function parseToolCallIdLoRaParts(args) {
    return {
...
export function formatIdLoRaPrompt(parts) {
    const lines = [`[VISUAL]: ${parts.visual}`];
    if (parts.speech)
        lines.push(`[SPEECH]: ${parts.speech}`);
    lines.push(`[SOUNDS]: ${parts.sounds}`);
    return lines.join('\n');
}
//# sourceMappingURL=idLoraPrompt.js.map