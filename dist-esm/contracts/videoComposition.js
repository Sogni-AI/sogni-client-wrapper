...
export const SCRIPT_COMPOSITION_TOOL = {
    type: 'function',
    function: {
        name: 'compose_script',
        ...
        parameters: {
            ...
            properties: {
                script: {
                    type: 'string',
                    description: 'The full cinematic video prompt as dense flowing prose. The scene ends when the script ends.',
                },
            },
            required: ['script'],
        },
...
function imageContentPart(dataUrl) {
    return { type: 'image_url', image_url: { url: dataUrl, detail: 'auto' } };
}
export function buildLtxScriptMessages(prompt, duration, options = {}) {
...
export function buildWanScriptMessages(params) {
...
export function parseToolCallScript(args) {
    return String(args.script || '').trim();
}
export function buildCharacterReferenceVideoCompositionMessages(input) {
    const content = [];
...
export function parseCompositionToolScriptFromResult(result) {
    const toolCalls = result?.tool_calls ?? [];
    for (const call of toolCalls) {
        if (call.function?.name !== 'compose_script')
            continue;
        try {
